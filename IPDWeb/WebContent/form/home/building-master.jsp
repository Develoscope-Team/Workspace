							...
							<form class="form" id="kt_form_1">
								<div class="card-body">
									...
									<div class="text-center">
										<button type="button" id="addBuilding"
											class="btn btn-primary mr-2"
											style="background-color: #AB48FF;">Submit</button>
									<button type="submit" id="updateBuilding"
												class="btn btn-primary mr-2"
...
<script type="text/javascript">
	...
	$("#addBuilding").show();
		...
		$('#addBuilding').click(function() {
							...
							var building_floors = $('#building_floors').val();
							var building_count = $('#building_count').val();
							var wards = $('#wards').val();
							var floors = $('#floors').val();
							var rooms = $('#rooms').val();
							var operation_theater = $('#operation_theater').val();
							var icu = $('#icu').val();
							var flag = 1; // Addition

						if(building_name != "" && building_floors != ""){
							$.ajax({
								// http://localhost:8080/ipdAPI/ipdapi/insertUpdateBuilding
								url : base +"/ipdAPI/ipdapi/insertUpdateBuilding",
								...
								success : function(response) {
									if (response != null) {
										...
										if (response >= 1) {
											...
											var msg = "Building Data inserted Successfully.";
											alert(msg);
...
$.ajax({
	...
	url : base +"/ipdAPI/ipdapi/getBuilding",
		 ...
		 function update(id){
				...
				$.ajax({
			        ...
			        success:function(data)
			        {
			            ...
			            const row = data.find(d => d.building_id ==id);
			           
			            $("#building_name").val(row.building_name);
			        	$("#building_floors").val(row.building_floors);
			        	$("#building_count").val(row.building_count);
			        	$("#wards").val(row.wards);
			        	$("#floors").val(row.floors);
			        	$("#rooms").val(row.rooms);
			        	$("#operation_theater").val(row.operation_theater);
			        	$("#icu").val(row.icu);
			        }
			     });
										...
										success : function(response) {
											if (response != null) {
												...
												if (response >= 1) {
													...
													var msg = "Building Data updated Successfully.";
													alert(msg);
				...
				success : function(response) {
					if (response != null) {
						...
						if (response >=1) {
							...
							var msg = "Building Data deleted Successfully.";
							alert(msg);
