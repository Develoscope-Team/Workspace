...
<body id="kt_body"
					...
					<div class="d-flex flex-column-fluid">
						...
						<div class="container d-flex align-items-stretch justify-content-between"">
							<div class="col-xl-12 offset-xl-1">
								<h2	class="d-flex align-items-center text-dark font-weight-bold my-1 mr-3 ml-4">Remark Master</h2>
	...
	$("#addRemark").show();
		$('#addRemark').click(function() {
							...
							var flag = 1; // Addition
						if(remark_name != "" && remark_desc != ""){
							$.ajax({
								url : base + "/dssAPI/dfapi/insertUpdateRemark",
								type : "post",
								dataType : "json",
								async : false,
								data : {
								...
								success : function(response) {
									if (response != null) {
										if (response >= 1) {
											var msg = "Remark Data inserted Successfully.";
...
var html = "";
$.ajax({
	url : "http://localhost:8080/dssAPI/dfapi/getRemark",
	type : "post",
	dataType : "json",
	async : false,
...
function update(id){
	$(window).scrollTop(0);
			...
			$.ajax({
		        ...
		        success:function(data)
		        {
		            const row = data.find(d => d.remark_id ==id);
		            $("#remark_name").val(row.remark_name);
		        	$("#remark_desc").val(row.remark_desc);
		        }
		     });
						...
						if(remark_name != '' && remark_desc != ''){
							$.ajax({
								...
								success : function(response) {
									if (response != null) {
										if (response >= 1) {
											var msg = "Remark Data Updated Successfully.";
											$('#success_msg').text(msg);
											 $('#success_alert').addClass("show");
									           $('#success_alert').removeClass("hide");
									           $('#success_alert').addClass("showAlert");
						...
						success : function(response) {
							if (response != null) {
								...
								if (response >=1) {
									...
									var msg = "Remark Data deleted Successfully.";
									$('#danger_msg').text(msg);
