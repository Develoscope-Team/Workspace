						...
						<div
							...
							<div class="d-flex align-items-center flex-wrap mr-1">
								...
								<div class="d-flex flex-column">
									<!--begin::Title-->
									<h2 class="text-white font-weight-bold my-2 mr-5">Purchase Entry</h2>
							...
							<div class="col-xl-12 offset-xl-0">
								...
								<div class=" ">
									<div class="row">
										...
										<div class="col-lg-6">
											<div class="form-group">
												<label>Delivery Date</label> <input type="date" data-date-inline-picker="true"
													class="form-control form-control-solid" id="delivery_date" />
											</div>
										</div>
									</div>
									<div class="row">
	...
	$("#mobile_no").change(function(){
	 ...
	 $.ajax({
			...
			success:function(data)
	     {
	         ...
	         $('#vendors_name').change(function(){
				...
				$.ajax({
	 			...
	 			for(var i = 1; i <= j; i++){
				 	  var html = '';
				 	  html += '<tr>'
				 	  html += ' <td><input type="checkbox" class="form-control checkbox Sr.No d-flex flex-column-fluid h-20px w-20px mt-3" id="id-'+i+'" onClick="checkedBox('+i+')"></td>';
	      ...
	      function calculator(i,valNum) {
    	      ...
    	      $('#grand_total').val(Grand_Total + parseInt(total_amount));     
			 var calculate_price_total = parseInt($("#partially_paid_amount").val());
	  for(var j = 1 ;j<=l ;j++)
		 {
		  
		  if($("#nowPaying-"+j).val() != null){
	 ...
	 var medData;
		...
		$('#add_user ').click(function () {
				...
				if(vendors_name != '' ){
					$.ajax({
						url : base + "/bakeshackAPI/api/insertUpdateVendors",
						type : "post",
						dataType : "json",
						async : false,
						data : {
...
$('#calculate_Product_total').click(function() {
	
	 var calculate_paid = 0;
	 var calculate_product_total =0;
	var k=0;
	 for(i=1; i < 100 ; i++)
	{
  
		 
	 var calculate_raw_qty = $("#raw_qty-"+i).val();
	 var calculate_unit_price = $("#unit_rate-"+i).val();
   calculate_product_total = parseInt(calculate_raw_qty) * parseInt(calculate_unit_price);
	 $("#unit_total-"+i).val(calculate_product_total); 
	 if(calculate_product_total > 0 )
		 {
		
	 calculate_paid = calculate_paid + calculate_product_total;
   
		 }
	} 
	 document.getElementById('amount').value = calculate_paid;
	 document.getElementById('total_amount').value = calculate_paid;
	 document.getElementById('partially_paid_amount').value = calculate_paid;
});
		...
		var jsonString = JSON.stringify(productList);
									...
									var latest_invoice_id;
									 ...
									 if ($("#id-"+i).prop('checked')==true){ 
										   ...
										   var field ="purchase_outstanding";
									   ...
									   $.ajax({
												...
												data : {
													  ...
													  "vendors_code"     : vendors_code,
														...
														"balance_amt"      : -a9,
														"invoice_code"     : a2,
														"field"            : "Outstanding Purchase Product",
														"transaction_code" : transaction_id,
														"flag"             : flag
												...
												success : function(response) {
													if (response != null) {
														...
														if (response >= 1) {
															...
															var msg = "Purchase Data Updated Successfully.";
															alert(msg);
										 ...
										 if (confirm("Do You Wish To Print Invoice!")) {
											  ...
											  //var url = "receipt.jsp";
										 		...
										 		var url = "purchase_receipt.jsp" + '?vendors_code=' + vendors_code + '&order_code=' + order_code + '&invoice_id=' + invoice_id;
