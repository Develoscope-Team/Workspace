		...
		success:function(data)
  {
	 ...
	 $('#vendors_name').change(function(){
			...
			for(var i = 1; i <= j; i++){
				 ...
				 /*  html += '<td>' + i + '</td>'; */
			 	 ...
			 	 // html += '<td><input type="text-center" class=" sales_order_id  " id="invoice_code-' + i +'" name="invoice_code" style="background-color:#FFFFFF; border:0px; text-align:center" disabled></td>';
			 	  ...
			 	  html += '<td><a type="button"   onClick="nextPage(' + i + ')";  id="nextPage-' + i +'" class="btn_edit btn font-weight-bold btn-primary btn-icon btn-primary text-left"  style="border:0px; width:50px; text-align:center; " ><i class="la la-edit"></i></a>'+
				...
				success:function(data)
		 	{
					...
					data.forEach((row)=> {
						...
						if(row.vendors_code == ccode )
								{
						  k++;
						  $('#sales_order_id-' + k).val(row.purchase_id);
							$('#invoice_code-' + k).val(row.invoice_code);
							$('#invoice_date-' + k).val(row.odrer_date);
		 ...
		 function nextPage(i){
              ...
              let purchase_id =  $('#sales_order_id-'+i).val();
		 	  ...
		 	  let order_code = $('#order_code-'+i).val();
		 		/* alert(vendors_name); */
		 		var url = "purchase-return-master.jsp" + '?vendors_name=' + vendors_name + '&vendors_code=' + vendors_code + '&purchase_id=' + purchase_id;
		 ...
		 function viewOrder(i){
	          ...
	          let order_code = $('#order_code-'+i).val();
		 		/* alert(customer_name); */
           var url = "purchase_receipt.jsp" + '?vendors_code=' + vendors_code + '&order_code=' + order_code + '&transaction_id=' + 0;
		 		//var url = "sales-return-master.jsp"  ;
