	 ...
	 $('#return_date').val(date);
				 ...
				 if($(this).find('.id').prop('checked')==true){
				    ...
				    $.ajax({
							...
							data : {
								...
								"opening_quantity" : 00,
								"purchase_quantity": 0,
								"purchase_return_quantity" : quantity,
								"used_quantity" : 0,
								"sale_quantity" : 0,
								"sale_return_quantity" : 0,
								"out_quantity": 0,
								"produced_quantity": 0,
								"transaction_field" : "PurReturnProduct",
								"flag" :flag
							},
							error : function(xhr) {
		...
		var jsonString = JSON.stringify(productList);
										...
										success : function(response) {
											if (response != null) {
												...
												if (response >= 1) {
													...
													var msg = "Purchase Return Data inserted Successfully.";
													 $('#success_msg').text(msg);
													 $('#success_alert').addClass("show");
											           $('#success_alert').removeClass("hide");
											           $('#success_alert').addClass("showAlert");
									...
									var transaction_id;
									 ...
									 $.ajax({
											url : base + "/bakeshackAPI/api/getLatestPurchaseReturnInvoice",
											type : "post",
											dataType : "json",
											async : false,
